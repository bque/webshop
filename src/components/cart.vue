<template>
	<div class="cart">
				...
				<ul class="shopGoods">
					<li class="clearfix shopGoodsLi" v-for="(item,indexs) in value.goods">
						...
						<div class="right relative">
							...
							<p><span class='opacity'>重量:{{item.weight}}</span> <span class="positionRight opacity">修改</span></p>
							<p class="lastP"><span class="money">￥{{item.price}}</span > <s class="referencePrice">$100</s><span class="positionRight"><a @click="changeNum(index,indexs,-1)">-</a><a>{{item.num}}</a><a @click="changeNum(index,indexs,1)">+</a></span> </p>
						</div>
					</li>
				</ul>
			</div>
		...
		<div class="bottom relative">
			...
			<a class="total">合计:<span class="money smallFont">￥{{totalMoney}}</span></a>
		...
		data() {
			return {
				...
				shoppingGoods: [{
						...
						checkStoreAll: false,
						goods: [{
								img: 'https://img.alicdn.com/imgextra/i1/59857264/TB2ItmtdaigSKJjSsppXXabnpXa_!!0-saturn_solar.jpg_220x220.jpg',
